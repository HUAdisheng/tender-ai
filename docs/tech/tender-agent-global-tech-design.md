...
- 当前产品形态统一按 `user` 主体建模，`user` 本身就是公司主体与登录主体。
...
当前阶段不单独引入“公司”和“用户”的关系建模，统一按 `user` 主体建模：

- `user` 本身就是公司主体，也是登录主体。
- `user` 拥有自己的项目、知识库、文件与生成结果。
- 所有数据默认按 `user_id` 归属隔离。

这样设计的原因：

- 当前阶段目标是尽快打通单公司使用闭环，不引入额外的主体关系复杂度。
- 可以避免公司表、成员表、组织角色表和租户隔离策略过早膨胀。
- 后续若确实需要多成员协作，可在现有 `user` 主体之上扩展协作模型，而不是现在提前做重。
...
所有业务接口统一返回 `application/json`，响应体建议使用如下结构：
...
- HTTP 状态码表达协议层结果，响应体 `code` 表达业务结果
- 成功响应不得直接裸返回业务对象、数组或字符串
- 列表接口放入 `data.items`，分页信息放入 `data.pagination`
- 异步任务型接口可在 `data` 中返回 `task_id`、`status`
...
`Tender Agent` 当前最适合采用 `Python` 分层单体架构，并以 `LangChain` 作为 AI 编排层：在部署上保持简单，在代码组织上先固定顶层骨架，再逐步补齐各层实现；业务主体统一按 `user` 建模，`user` 即公司主体；文件统一落到 `rustfs`；当前阶段暂不启用图库功能。该方案能更稳定地支撑 `MVP` 快速落地，同时为后续能力扩展保留清晰边界。
